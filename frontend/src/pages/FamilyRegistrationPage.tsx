...
import { submitFamily } from '../services/api';
...
export default function FamilyRegistrationPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await submitFamily({ name: name.trim(), phone_number: `+91${phone}`, age_category: ageCategory || undefined, business_name: businessName || undefined, business_category: businessCategory || undefined, photo: photoFile });
      toast.success('Family registration successful');
      setName(''); setPhone(''); setAgeCategory(''); setBusinessName(''); setBusinessCategory(''); setPhotoFile(null); setPhotoPreview(null);
    } catch (err: any) {
  ...
  return (
    <>
      <Navbar />
      <div className="page-header"><h1>SPOUSE & KIDS REGISTRATION</h1></div>
