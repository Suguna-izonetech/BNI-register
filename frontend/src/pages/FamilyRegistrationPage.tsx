...
export default function FamilyRegistrationPage() {
  ...
  const validate = () => {
    ...
    if (!ageCategory) return 'Age category is required';
          ...
          <div class="admit-details">
            ...
            <div class="admit-fields">
              ...
              <div class="section">
                ...
                <div class="field"><span class="label">Business Name:</span><span class="value">${submittedData.business_name}</span></div>
                <div class="field"><span class="label">Business Category:</span><span class="value">${submittedData.business_category}</span></div>
              </div>
            </div>
          </div>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await submitFamily({ team_name: teamName, name: name.trim(), phone_number: `+91${phone}`, age_category: ageCategory, business_name: businessName.trim(), business_category: businessCategory.trim(), photo: photoFile });
      setSubmittedData({
        team_name: teamName,
        name: name.trim(),
        ...
        <div className="form-container">
          <div className="form-card" style={{ textAlign: 'center', padding: '3rem 2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              onClick={() => { setTeamName(''); setName(''); setPhone(''); setAgeCategory(''); setBusinessName(''); setBusinessCategory(''); setPhotoFile(null); setPhotoPreview(null); setSubmitted(false); setSubmittedData(null); }}
            >
              ➕ REGISTER ANOTHER
              </button>
            </div>
  ...
  return (
    <>
      <Navbar />
      <div className="page-header"><button className="btn-secondary" onClick={() => navigate('/')} style={{ marginBottom: '0.75rem', color: 'var(--white)', borderColor: 'var(--white)' }}>← Back to Home</button><h1>SPOUSE & KIDS REGISTRATION</h1></div>
      <div className="form-container">
